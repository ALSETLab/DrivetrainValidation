...
model MultiRotor_ColdPlate
  ...
  Battery.Packs.Scaled.ScaledPackCylindric batteryPack(
    ...
    SOC_init=1,
    T_init=293.15)                               annotation (Placement(transformation(extent={{-14,-14},
            {14,14}},
        rotation=0,
        origin={4,-4})));
  ...
  Battery.Packs.ThermalBoundaries.ConstantTemp
                               constantTemp(
    packType=batteryPack.packType,
    N_x=batteryPack.N_x,
    N_y=batteryPack.N_y,
    N_z=batteryPack.N_z,
    T_top(displayUnit="K") = 293.15,
    T_bottom=constantTemp.T_top,
    T_front=constantTemp.T_top,
    T_back=constantTemp.T_top,
    T_left=constantTemp.T_top,
    T_right=constantTemp.T_top)        annotation (Placement(transformation(extent={{-10,56},
            {18,84}})));
equation
  ...
  connect(constantTemp.housingHeatPort, batteryPack.housingHeatPort)
    annotation (Line(points={{4,56},{4,10}}, color={191,0,0}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{
            -100,-120},{180,100}})), Diagram(coordinateSystem(
          preserveAspectRatio=false, extent={{-100,-120},{180,100}})));
end MultiRotor_ColdPlate;
