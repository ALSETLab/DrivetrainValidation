...
model MultiRotor_Temp
  ...
  Battery.Packs.ThermalBoundaries.Convection convection(
    packType=batteryPack.packType,
    N_x=batteryPack.N_x,
    N_y=batteryPack.N_y,
    N_z=batteryPack.N_z)
    annotation (Placement(transformation(extent={{-10,56},{18,84}})));
equation
  ...
  connect(convection.housingHeatPort, batteryPack.housingHeatPort)
